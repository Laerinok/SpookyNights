...
namespace SpookyNights
{
  ...
  public sealed class SpookyNightsModSystem : ModSystem
  {
    ...
    public override void StartServerSide(ICoreServerAPI api)
    {
      ...
      // /findspooky : Finds SpookyNights structures within a given radius
      api.ChatCommands.GetOrCreate("findspooky")
       .WithDescription("Finds SpookyNights structures within a radius (Admins only)")
       .WithArgs(api.ChatCommands.Parsers.OptionalInt("radius", 2000))
       ...
       .HandleWith(args =>
       {
         ...
         BlockPos spawnPos = api.World.DefaultSpawnPosition.AsBlockPos;

         int minRx = (pPos.X - radius) / regionSize;
         int maxRx = (pPos.X + radius) / regionSize;
         int minRz = (pPos.Z - radius) / regionSize;
         int maxRz = (pPos.Z + radius) / regionSize;
         ...
         for (int rx = minRx; rx <= maxRx; rx++)
         {
           ...
           {
             IMapRegion region = api.WorldManager.GetMapRegion(rx, rz);
             if (region?.GeneratedStructures == null) continue;

             foreach (var struc in region.GeneratedStructures)
             {
               if (struc.Code.Contains("spookynights"))
               {
                 Vec3i c = struc.Location.Center;

                 // Distance calculation must use absolute coordinates
                 double dist = Math.Sqrt(pPos.DistanceSqTo(c.X, c.Y, c.Z));

                 if (dist <= radius)
                 {
                   // Calculate values matching your HUD and /tp usage
                   int relX = c.X - spawnPos.X;
                   int absY = c.Y; // Keep absolute height (e.g. 114)
                   int relZ = c.Z - spawnPos.Z;

                   // Display and Link now use identical relative/HUD coordinates
                   sb.AppendLine($"- {struc.Code} : <a href=\"chattype:///tp {relX} {absY} {relZ}\">X={relX}, Y={absY}, Z={relZ}</a> ({Math.Round(dist)}m)");
                   count++;
                 }
               }
             }
         ...
         if (count == 0) return TextCommandResult.Success($"No structures found within {radius} blocks.");
         return TextCommandResult.Success($"Found {count} structures:\n" + sb.ToString());
       });
    ...
    private void OnDaylightCheck(float dt)
    {
      ...
      var conf = ConfigManager.ServerConf;
      if (!conf.UseTimeBasedSpawning) return;
